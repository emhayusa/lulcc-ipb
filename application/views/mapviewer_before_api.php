...
<html lang="en">
  <head>
    ...
    <title>Land Cover Change Monitoring System</title>
...
<script type="text/javascript">
 ...
 var urlListKSP = "<?php echo base_url("api/listLayer");?>";
 var urlListJIGN = "<?php echo base_url("api/listJIGN");?>";
 var bypass = "<?php echo base_url("api/bypass");?>";
	...
	function setup_component(){
		...
		$("#mainMenu").append('<div id="ksp"></div>');
	  	...
	  	servicesArray.forEach(function(obj) {
		    ...
		    $("#ksp-"+obj.id).append('<img src="assets/images/'+obj.logo+'" width="46px" height="46px" title="'+obj.name+'">');
  ...
  <option value="100"></option>
  	...
  	function populateExtraMenuTools(){
		...
		*/
		...
		$('#containerHelp').append('<div class="text-center"><h5>Land Cover Change Monitoring System<br />to Ensure Responsible Land-Use Arrangement for Sustainable Development</h5></div><br/><br/>');
		$('#containerHelp').append('<ul><li>The UNDP-GEF project, Support to Reduced Deforestation Commodity Production, (hereafter the ‘Production project’) is a child project under the UNDP-GEF 6 Integrated Approach Pilot (IAP) program, Taking Deforestation out of Commodity Supply Chains.</li><li>The IAP program is advancing an integrated, supply chain approach to tackling the underlying root causes of deforestation* from agriculture commodities, specifically beef, palm oil, and soy that together account for nearly 70% of deforestation globally.</li><li>This approach consists of linked projects covering production, demand, transactions and knowledge management and learning. The Production project will contribute to addressing the challenge by concentrating on the production of two of the main commodities driving these worrisome trends: palm oil and beef, in three target countries, Indonesia, Liberia, and Paraguay</li></ul>');
		$('#containerHelp').append('<div class="text-center"><img src="assets/images/GGP.png" width="480px" /></div>');
    }
                 	        ...
                 	        success: function(json) {
    		                     ...
    		                     $.each(json.folders, function( key, val ) {
    				    			...
    				    			var url = sp[1];
    		                     	...
    		                     	$.post("api/arcgis",{protocol: protocol, url: url}).done(function(data) {
    										//console.log(data);
	...
	*/
	...
	function init_widget(){
		...
		var localSource = new LocalBasemapsSource({
            	basemaps : [rbi, nkri, Basemap.fromId("topo"), Basemap.fromId("osm"),Basemap.fromId("satellite"),Basemap.fromId("streets-navigation-vector")]
			})
